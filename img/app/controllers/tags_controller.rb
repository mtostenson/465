...
class TagsController < ApplicationController
  ...
  # that way the Rating's foreign key (image_id) will be
  ...
  def new
    ...
    # This is similar to Rating.new, BUT it creates the new tag
  ...
  def create
    ...
    if @tag.save
      redirect_to image_tags_url(@image) , notice: 'Rating was successfully created.'
    else
      render :new
    end
  end
  ...
  def update
    if @tag.update(tag_params)
      redirect_to image_tags_url(@tag.image), notice: 'Rating was successfully updated.'
    else
      render :edit
    end
  end
  ...
  def destroy
    @tag.destroy
    redirect_to image_tags_url(@tag.image) , notice: 'Rating was successfully destroyed.'
  end
  ...
  private
    ...
    def tag_params
      params.require(:tag).permit(:course, :comment, :tag)
    end
end